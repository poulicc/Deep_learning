...
from verif_function import reconstruction_sig_dB, SNR
...
if NETWORK_TYPE=="CNN":
    TRAINNOISEPATH=os.path.join("..", "..", "network", "Dataset_spectro_dB","Train","Noisy","*.npy") #noisy spectro train
    TRAINCLEANPATH=os.path.join("..", "..", "network", "Dataset_spectro_dB", "Train","Clean","*.npy") #clean spectro train

    VALNOISEPATH=os.path.join("..", "..", "network", "Dataset_spectro_dB","Val","Noisy","*.npy")#noisy spectro val
    VALCLEANPATH=os.path.join("..", "..", "network", "Dataset_spectro_dB", "Val","Clean","*.npy")#clean spectro val

if NETWORK_TYPE=="FNN":
    TRAIN_X_NOISEPATH=os.path.join("..", "..", "network", "Dataset", "Train","Noisy","X_all_dB.npy")#noisy spectro train FILE
    TRAIN_X_CLEANPATH=os.path.join("..", "..", "network", "Dataset", "Train","Clean","X_clean_all_dB.npy")#clean spectro train FILE

    VAL_X_NOISEPATH=os.path.join("..", "..", "network", "Dataset", "Val","Noisy","X_all_dB.npy")#noisy spectro val FILE
    VAL_X_CLEANPATH=os.path.join("..", "..", "network", "Dataset", "Val","Clean","X_clean_all_dB.npy")#clean spectro val FILE

TRAIN_PHASE_NOISEPATH=os.path.join("..", "..", "network", "info","Train", "X_phase_all_dB.npy") #train phase noise
TRAIN_PHASE_CLEANPATH=os.path.join("..", "..", "network", "info","Train", "X_clean_phase_all_dB.npy")#train phase clean

VAL_PHASE_NOISEPATH=os.path.join("..", "..", "network", "info","Val", "X_phase_all_dB.npy")#val phase noise
VAL_PHASE_CLEANPATH=os.path.join("..", "..", "network", "info","Val", "X_clean_phase_all_dB.npy")#val phase clean

TRAIN_NORM_PATH=os.path.join("..", "..", "network", "info","Train", "normalize_info_all_dB.npy") #norm info train
VAL_NORM_PATH=os.path.join("..", "..", "network", "info","Val", "normalize_info_all_dB.npy") #norm info val
...
if VERIF_TRAIN:
    ...
    if PRINT_INFO:
        ...
        if PRINT_SPECTRO_DB:
            plt.figure()
            plt.subplot(121)
            plt.pcolormesh(10**(noisy_spectro/20))
            plt.colorbar()
            plt.title("Spectrogram of the new noisy signal, lin")

            plt.subplot(122)
            plt.pcolormesh(10**(clean_spectro/20))
            plt.colorbar()
            plt.title("Spectrogram of the clean signal, lin")
            plt.show()

    if RECONSTRUC:
        recons_noisy=reconstruction_sig_dB(noisy_spectro, noisy_phase, max, min,True,[True, str(index)+"_noisy.flac"])
        recons_clean=reconstruction_sig_dB(clean_spectro, clean_phase, max, min,True,[True, str(index)+"_clean.flac"])
