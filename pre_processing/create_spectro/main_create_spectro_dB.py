...
for index in range (0, len(train_noise_paths)):
    ...
    noisy, _= sf.read(train_noise_paths[index])
    clean, _ = sf.read(train_clean_paths[index])
    ...
    if SAVE_TRAIN:
        SAVENOISEPATH=os.path.join("..", "..", "network", "Dataset_spectro_dB","Train","Noisy")
        SAVECLEANPATH=os.path.join("..", "..", "network", "Dataset_spectro_dB", "Train","Clean")
        namefile=str(list)
...
for index in range (0, len(val_noise_paths)):
    ...
    if SAVE_VAL:
        SAVENOISEPATH=os.path.join("..", "..", "network", "Dataset_spectro_dB","Val","Noisy")
        SAVECLEANPATH=os.path.join("..", "..", "network", "Dataset_spectro_dB", "Val","Clean")
        namefile=str(list)
...
if NB_SAMPLES == "all" :
    ...
    TRAIN_NOISEPHASE=os.path.join(DATADIRPATH,"Train","X_phase_all_dB")
    TRAIN_CLEANPHASE=os.path.join(DATADIRPATH,"Train","X_clean_phase_all_dB")
    TRAIN_NORMA=os.path.join(DATADIRPATH,"Train","normalize_info_all_dB")

    VAL_NOISEPHASE=os.path.join(DATADIRPATH,"Val","X_phase_all_dB")
    VAL_CLEANPHASE=os.path.join(DATADIRPATH,"Val","X_clean_phase_all_dB")
    VAL_NORMA=os.path.join(DATADIRPATH,"Val","normalize_info_all_dB")

    if NETWORK_TYPE=="FNN":
        DATADIRPATH2=os.path.join(CURDIRPATH,"..","..","network", "Dataset")
        TRAIN_NOISEPATH=os.path.join(DATADIRPATH2,"Train", "Noisy","X_all_dB")
        TRAIN_CLEANPATH=os.path.join(DATADIRPATH2,"Train","Clean","X_clean_all_dB")
        VAL_NOISEPATH=os.path.join(DATADIRPATH2,"Val","Noisy","X_all_dB")
        VAL_CLEANPATH=os.path.join(DATADIRPATH2,"Val","Clean","X_clean_all_dB")
